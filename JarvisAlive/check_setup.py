...
def check_env_file():
    ...
    if 'ANTHROPIC_API_KEY=' in content and len(content.split('ANTHROPIC_API_KEY=')[1].split('\n')[0].strip()) > 10:
        print("✅ Anthropic API key configured in .env")
        return True
    else:
        print("❌ Anthropic API key not configured in .env")
        return False
