...
class HeyJarvisOrchestrator:
    ...
    async def _understand_intent(self, state: OrchestratorState) -> Dict[str, Any]:
        ...
        try:
            ...
            - CLARIFICATION_NEEDED: Request is too vague and needs clarification
            ...
            CONFIDENCE RULES:
            - High confidence (0.8-1.0): Clear, specific requests
            - Medium confidence (0.5-0.7): Somewhat clear but missing details
            - Low confidence (0.0-0.4): Vague, ambiguous, or unclear
            ...
            needs_clarification = (
                parsed_intent["confidence"] < 0.7 or
                intent_data["intent_type"] == "CLARIFICATION_NEEDED" or
                bool(intent_data.get("clarification_needed", {}).get("questions"))
            )
        ...
        try:
            ...
            if agent_spec.get("config", {}).get("pydantic_spec"):
                try:
                    ...
                    generated_code = await generate_agent_code(pydantic_spec, self.config.openai_api_key)
